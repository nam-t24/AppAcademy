...
def ford_supporting_films
  ...
  execute(<<-SQL)
    ...
    JOIN
      actors ON castings.actor_id = actors.id
    WHERE
      castings.ord != 1 AND actors.name LIKE 'Harrison Ford'
  SQL
end
    ...
    WHERE
      (
        ...
        FROM
          castings
        WHERE
          ord = 1 AND actors.id = castings.actor_id
      ) >= 15
    ORDER BY
      actors.name ASC
  SQL
...
def colleagues_of_garfunkel
  ...
  execute(<<-SQL)
    ...
    JOIN
      ...
      ) AS art_actors ON art_movies.id = art_actors.movie_id
  SQL
end
